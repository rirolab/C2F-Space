...
def get_final_pretrained_ckpt(ckpt_dir, ckpt_idx=-1):
    if osp.exists(ckpt_dir):
        ...
        if ckpt_idx < 0:
            ...
            final_epoch = ckpt_idx
        # final_epoch = ckpt_idx # max(epochs)
    else:
...
def init_model_from_pretrained(model, pretrained_dir, freeze_pretrained=False, ckpt_idx=-1):
    ...
    """
    ckpt_file = get_final_pretrained_ckpt(osp.join(pretrained_dir, '0', 'ckpt'), ckpt_idx=ckpt_idx)
