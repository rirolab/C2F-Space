...
class Environment(gym.Env):
    ...
    def __init__(self,
        ...
        """
        self.scene_graph_generator = SceneGraphWithGDino()
    ...
    def reset(self):
        ...
        plane_id = pybullet_utils.load_urdf(
            p, os.path.join(self.assets_root, UR5_WORKSPACE_URDF_PATH),
            [0.5, 0, 0]
        )
        # load texture
        texture_file = os.path.join(self.assets_root, 'plane/wood.jpg')
    ...
    def add_video_frame(self):
        ...
        color_ = color_
        if self.place_pix is not None:
            color_ = cv2.circle(
                color_.astype(np.float32), (self.place_pix[0], self.place_pix[1]), 5, (255, 0, 0), -1
            )
        color_ = cv2.resize(
    ...
    def get_urdf_size(self, urdf, angle):
        obj_id = pybullet_utils.load_urdf(
            p,
            os.path.join(self.assets_root, urdf),
            [-3.0, -3.0, 1.0],
        ...
        z_c = (boundaries[0][2] + boundaries[1][2])/2
        x_d = -3.0-x_c
        y_d = -3.0-y_c
        z_d = 1.0-z_c
