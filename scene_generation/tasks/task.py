...
from scene_generation.utils.global_vars import BOUNDS, PIXEL_SIZE
from scene_generation.utils import cameras
from scene_generation.tasks import primitives
from scene_generation.tasks.grippers import Suction
    ...
    def fill_template(self, template, replace):
        ...
        for field in replace:
            for i in range(len(replace[field])):
                if ':/' in field:
                    fdata = fdata.replace(field, replace[field])
                else:
                    for i in range(len(replace[field])):
