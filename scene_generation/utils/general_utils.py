...
import random
from scene_generation.utils.global_vars import BOUNDS, CAMERA_CONFIG, PIXEL_SIZE
...
def raw_pix_to_xyz(pixel, depth, config, image_size):
    def scale_intrinsics(intrinsics, orig_size, new_size):
        focal_len = intrinsics[0, 0]
        fovh = (new_size[0] / 2) / focal_len
        fovh = 180 * np.arctan(fovh) * 2 / np.pi

        # Notes: 1) FOV is vertical FOV 2) aspect must be float
        aspect_ratio = new_size[1] / new_size[0]
        fy = new_size[0] / (2 * np.tan(fovh * np.pi / 360))
        # fx = fy * aspect_ratio
        ...
        return scaled
    intrinsics = np.array(config['intrinsics']).reshape(3, 3)
    intrinsics = scale_intrinsics(intrinsics, config['image_size'], image_size)
    u, v = pixel
    # v = image_size[0] - 1 - v
    ...
    y = (v - cy) * depth / fy
    z = depth
    xyz_cam = np.array([x, y, z, 1.0], dtype=np.float32)  # homogeneous
    position = np.array(config['position']).reshape(3, 1)
    rotation = p.getMatrixFromQuaternion(config['rotation'])
    rotation = np.array(rotation).reshape(3, 3)
    transform = np.eye(4)
